...
function safeClass(value, allowed) { return allowed.includes(value) ? value : allowed[0]; }
...
function renderProgress(job) { return `<progress class="progress-native" max="100" value="${progressPercent(job)}" aria-label="İlerleme ${progressPercent(job)} yüzde"></progress>`; }
...
async function loadCurrentUser() { try { updateAuthUi(await api('/api/auth/me')); } catch { clearToken(); updateAuthUi({ id: 'local', email: 'local@app', full_name: 'Local Kullanıcı' }); } }
...
function setActiveCategory(category) { activeCategory = category; $$('.subtab').forEach(btn => btn.classList.toggle('active', btn.dataset.category === category)); const emailCategory = $('#emailAlertForm [name="default_category"]'); if (emailCategory) emailCategory.value = category; syncCategoryFields(); }
function openTab(tabName) { $$('.tab').forEach(b => b.classList.toggle('active', b.dataset.tab === tabName)); $$('.panel').forEach(p => p.classList.toggle('active', p.id === tabName)); if (tabName === 'sources') loadSources(); if (tabName === 'jobs') loadJobs(); }
function bindTabs() { $$('.tab').forEach(btn => btn.addEventListener('click', () => openTab(btn.dataset.tab))); $$('[data-open-tab]').forEach(btn => btn.addEventListener('click', () => openTab(btn.dataset.openTab))); $$('.subtab').forEach(btn => btn.addEventListener('click', () => { setActiveCategory(btn.dataset.category); loadListings(); loadSavedSearches(); })); }
...
function bindEmailAlertForm() { const form = $('#emailAlertForm'); if (!form) return; form.addEventListener('submit', async (e) => { e.preventDefault(); const raw = formDataToObject(e.currentTarget); try { const result = await api('/api/import/email-alert', { method: 'POST', body: JSON.stringify({ text: raw.text, default_category: raw.default_category || activeCategory, default_city: raw.default_city || '', default_district: raw.default_district || '' }) }); showToast(`${result.imported_count} ilan aktarıldı, ${result.skipped_count} atlandı.`); e.currentTarget.reset(); await loadListings(); } catch { showToast('E-posta alarmı aktarılamadı. Metinde link ve fiyat olduğundan emin ol.'); } }); }
function bindListingForm() {
  ...
  $('#makeLinksBtn').addEventListener('click', async () => { const city = $('#filterCity').value || $('#listingForm [name="city"]').value || ''; const district = $('#filterDistrict').value || $('#listingForm [name="district"]').value || ''; const neighborhood = $('#listingForm [name="neighborhood"]').value || ''; const links = await api('/api/search-links', { method: 'POST', body: JSON.stringify({ category: activeCategory, city, district, neighborhood, keywords: $('#filterQ').value || '' }) }); $('#searchLinks').innerHTML = links.map(x => { const url = safeUrl(x.url); return url ? `<a href="${url}" target="_blank" rel="noopener noreferrer">${escapeHtml(x.source)}: ${url}<br><small>${escapeHtml(x.note)}</small></a>` : ''; }).join(''); });
  $('#listings').addEventListener('click', async (e) => { const target = e.target.closest('[data-action]'); if (!target) return; const id = Number(target.dataset.id); if (target.dataset.action === 'favorite') { const current = target.dataset.current === 'true'; await api(`/api/listings/${id}/favorite`, { method: 'PATCH', body: JSON.stringify({ is_favorite: !current }) }); showToast(!current ? 'Favorilere eklendi.' : 'Favorilerden çıkarıldı.'); await loadListings(); } if (target.dataset.action === 'delete') { if (!confirm('Bu ilan silinsin mi?')) return; await api(`/api/listings/${id}`, { method: 'DELETE' }); showToast('İlan silindi.'); await loadListings(); } });
}
...
async function syncAllSourcesNow(button) { button.disabled = true; button.textContent = 'İş oluşturuluyor...'; try { const result = await api('/api/jobs/source-sync-all', { method: 'POST' }); showToast(`Arka plan işi oluşturuldu: #${result.job_id}`); await loadJobs(); showJobDetail(result.job_id); } catch { showToast('Arka plan işi oluşturulamadı.'); } finally { button.disabled = false; button.textContent = 'Tümünü senkronize et'; } }
function bindSources() { $('#syncAllSourcesBtn').addEventListener('click', (e) => syncAllSourcesNow(e.currentTarget)); $('#sourceForm').addEventListener('submit', async (e) => { e.preventDefault(); const raw = formDataToObject(e.currentTarget); await api('/api/data-sources', { method: 'POST', body: JSON.stringify({ name: raw.name, source_type: raw.source_type, url: raw.url, category: raw.category, enabled: true, config: {} }) }); e.currentTarget.reset(); showToast('Veri kaynağı eklendi.'); await loadSources(); }); $('#sourceList').addEventListener('click', async (e) => { const target = e.target.closest('[data-source-action]'); if (!target) return; const id = Number(target.dataset.id); if (target.dataset.sourceAction === 'sync') { target.disabled = true; target.textContent = 'Senkronize ediliyor...'; try { const result = await api(`/api/data-sources/${id}/sync`, { method: 'POST' }); showToast(`${result.imported_count} yeni ilan eklendi, ${result.skipped_count} tekrar atlandı.`); await loadSources(); await loadListings(); } catch { showToast('Senkronizasyon başarısız. Kaynak durumunu kontrol et.'); await loadSources(); } } if (target.dataset.sourceAction === 'delete') { if (!confirm('Veri kaynağı silinsin mi? Daha önce gelen ilanlar silinmez.')) return; await api(`/api/data-sources/${id}`, { method: 'DELETE' }); showToast('Veri kaynağı silindi.'); await loadSources(); } }); }
...
async function loadJobs() { const jobs = await api('/api/jobs?limit=30'); if (!jobs.length) { $('#jobList').innerHTML = '<div class="empty-state"><b>Henüz iş yok.</b><p>Veri Kaynakları sekmesinden toplu senkronizasyon başlatabilirsin.</p></div>'; return; } $('#jobList').innerHTML = jobs.map(job => { const cls = safeClass(job.status, ['queued', 'running', 'succeeded', 'failed']); return `<article class="job-card ${cls}"><span class="source-icon">▤</span><div><b>#${Number(job.id)} · ${escapeHtml(job.title)}</b><span>${escapeHtml(jobStatusLabels[job.status] || job.status)} · ${escapeHtml(progressText(job))}</span>${renderProgress(job)}</div><button data-job-id="${Number(job.id)}">Detay</button></article>`; }).join(''); }
async function showJobDetail(jobId) { const [job, events] = await Promise.all([api(`/api/jobs/${Number(jobId)}`), api(`/api/jobs/${Number(jobId)}/events`)]); $('#jobDetail').innerHTML = `<div class="job-detail-card"><h3>İş #${Number(job.id)}</h3><p><b>Durum:</b> ${escapeHtml(jobStatusLabels[job.status] || job.status)}</p><p><b>İlerleme:</b> ${escapeHtml(progressText(job))}</p>${job.error ? `<p class="danger-text"><b>Hata:</b> ${escapeHtml(job.error)}</p>` : ''}${renderProgress(job)}<h4>Olaylar</h4>${events.length ? events.map(ev => `<div class="job-event ${safeClass(ev.level, ['info', 'warning', 'error'])}"><b>${escapeHtml(ev.created_at || '')}</b><span>${escapeHtml(ev.message)}</span></div>`).join('') : '<p class="muted">Henüz olay yok.</p>'}</div>`; if (activeJobPoll) clearInterval(activeJobPoll); if (['queued', 'running'].includes(job.status)) { activeJobPoll = setInterval(async () => { await loadJobs(); await showJobDetail(jobId); const fresh = await api(`/api/jobs/${Number(jobId)}`); if (!['queued', 'running'].includes(fresh.status)) { clearInterval(activeJobPoll); activeJobPoll = null; await loadSources(); await loadListings(); } }, 2500); } }
...
function renderMeclisResult(result) { $('#meclisSummary').hidden = false; $('#meclisSummary').innerHTML = `<b>${Number(result.matched_keywords)} anahtar kelime eşleşti</b><span>${Number(result.total_hits)} toplam eşleşme · ${Number(result.text_length)} karakter</span>${result.pdf ? `<span>PDF: ${escapeHtml(result.pdf.filename)} · ${Number(result.pdf.page_count)} sayfa${result.pdf.needs_ocr ? ' · OCR gerekebilir' : ''}</span>` : ''}`; const rows = result.keywords || []; if (!rows.length) { $('#meclisResults').innerHTML = '<div class="empty-state"><b>Anahtar kelime yok.</b></div>'; return; } $('#meclisResults').innerHTML = rows.map(row => `<article class="meclis-card ${row.found ? 'hit' : 'miss'}"><div class="meclis-card-head"><b>${escapeHtml(row.keyword)}</b><span>${Number(row.count)} eşleşme</span></div>${row.contexts?.length ? row.contexts.map(ctx => `<p>${escapeHtml(ctx)}</p>`).join('') : '<p class="muted">Bu kelime bulunamadı.</p>'}</article>`).join(''); }
