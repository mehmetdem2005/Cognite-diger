...
CREATE POLICY "Reader exercises public read" ON public.reader_exercises FOR SELECT USING (TRUE);
...
FOR SELECT USING (
  EXISTS (SELECT 1 FROM public.reading_group_members gm WHERE gm.group_id = reading_group_members.group_id AND gm.user_id = auth.uid())
  OR EXISTS (SELECT 1 FROM public.reading_groups g WHERE g.id = reading_group_members.group_id AND g.is_public = TRUE)
);

DROP POLICY IF EXISTS "Group members self join" ON public.reading_group_members;
CREATE POLICY "Group members self join" ON public.reading_group_members FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Group members self leave" ON public.reading_group_members;
CREATE POLICY "Group members self leave" ON public.reading_group_members FOR DELETE USING (auth.uid() = user_id);
...
FOR SELECT USING (
  EXISTS (SELECT 1 FROM public.reading_group_members gm WHERE gm.group_id = reading_group_messages.group_id AND gm.user_id = auth.uid())
);
...
FOR INSERT WITH CHECK (
  auth.uid() = user_id
  AND EXISTS (SELECT 1 FROM public.reading_group_members gm WHERE gm.group_id = reading_group_messages.group_id AND gm.user_id = auth.uid())
);
